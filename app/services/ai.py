...
def generate_quiz_questions(
    ...
    system_prompt = (
        ...
        "  * ALWAYS wrap EVERY mathematical variable, unit, and formula in dollar signs (e.g. `$m = 0.20\\text{ kg}$`, `$k = 80\\text{ N m}^{-1}$`, `$A = 0.05\\text{ m}$`, `$\\omega_d = \\sqrt{\\omega_0^2 - (r/2m)^2}$`, `$2.5\\times 10^{-2}\\text{ m}$`, `$$H_c(T) = H_0 \\left[1 - \\left(\\frac{T}{T_c}\\right)^2\\right]$$`).\n"
